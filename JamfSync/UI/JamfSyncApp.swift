...
final class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    func cleanup() {
        ...
        if group.wait(timeout: DispatchTime.now() + timeout) != .success {
            ...
            alert.informativeText = "You may need to unmount these manually using Finder or locate the volume name in /Volumes and run \"diskutil unmount /Volumes/name\" from terminal."
            alert.addButton(withTitle: "OK")
            alert.alertStyle = .warning
            alert.runModal()
        }
